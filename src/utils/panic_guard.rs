//! Panic-to-error boundary for async operations.
//!
//! Third-party parsers (pdf-extract / lopdf, docx-rs, calamine, …) are
//! known to panic on malformed input instead of returning Err. These helpers
//! catch panics and convert them into Result::Err for graceful error handling.

use std::any::Any;
use std::panic::{catch_unwind, AssertUnwindSafe};

/// Run a synchronous function body, converting any panic into an Err.
...
/// Run an async function body, converting any panic into an Err.
...
fn report(label: &str, payload: Box<dyn Any + Send>) -> String {
    ...
    eprintln!("[panic_guard] operation '{label}' panicked: {msg}");
